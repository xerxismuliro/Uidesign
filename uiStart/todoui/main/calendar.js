...
document.addEventListener('DOMContentLoaded', () => {
  ...
  searchBox.addEventListener('input', () => {
    const searchTerm = searchBox.value.toLowerCase(); const filterRows = (tableBody) => {const rows = tableBody.getElementsByTagName('tr');
...
} function setupIndexedDB() {return new Promise((resolve, reject) => {
...
} function setupAddEventForm(db) {const addEventForm = document.getElementById('addEventForm');
  addEventForm.addEventListener('submit', (event) => {
    event.preventDefault();
    addEventToIndexedDB(db);
  });
} function addEventToIndexedDB(db) {const newEvent = {
...
} function loadBrowserEvents(db) {const transaction = db.transaction(['events'], 'readonly');
  const objectStore = transaction.objectStore('events');
  ...
  request.onsuccess = (event) => {
    ...
    events.forEach((event) => {
      ...
      const formattedTimeCreated = new Date(event.timeCreated).toLocaleString();
      const formattedEventTime = new Date(`1970-01-01T${event.time}Z`).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true });
...
} function setupSectionNavigation(db) {const sections = document.querySelectorAll('.left-apps .section > div, .allINtopBar > div');
...
} function displayMonthsWithEvents(db) {const monthsWithEventsDiv = document.getElementById('MonthsWthEvents-leftDiv');
  if (!monthsWithEventsDiv) {
    console.error('Element with id "MonthsWthEvents-leftDiv" not found.');
    return;
  }
  monthsWithEventsDiv.innerHTML = '';
...
} function getTerminalEvents() {const terminalEventsContentDiv = document.querySelector('#terminalEventsContent tbody');
